...
const NotificationSettingsScreen = () => {
	...
	const [testNotificationId, setTestNotificationId] = useState<string | null>(
		null,
	);
	...
	const updateSetting = async (
		...
		try {
			...
			setAlertMessage(t('notification.failed_update_notification_settings'));
			...
			},
			{
				...
				onPress: async () => {
					try {
						...
						setAlertTitle('Error');
						setAlertMessage(t('notification.failed_reset_notifications'));
						setAlertActions([{ label: 'OK', onPress: () => setAlertVisible(false) }]);
						setAlertVisible(true);
					}
				},
	...
	const handleSyncNotifications = async () => {
		...
		try {
			...
			setAlertTitle(t('common.error'));
			setAlertMessage(t('notification.failed_sync_notifications'));
			setAlertActions([{ label: 'OK', onPress: () => setAlertVisible(false) }]);
			setAlertVisible(true);
		} finally {
			setIsSyncing(false);
	...
	const handleTestNotification = async () => {
		try {
			...
			const testNotifications = scheduled.filter((n) =>
				n.id.startsWith('test-notification-'),
			);
			if (
			...
			if (
				...
				await notificationService.updateSettings({
					timeBeforeAiring: originalTimeBeforeAiring,
				});
			}
			...
			if (notificationId) {
				...
				setAlertTitle(t('common.error'));
				setAlertMessage(t('notification.failed_test_enable_notifications'));
				setAlertActions([{ label: 'OK', onPress: () => setAlertVisible(false) }]);
				setAlertVisible(true);
			}
		} catch (error) {
			...
			setAlertTitle(t('common.error'));
			setAlertMessage(t('notification.failed_test'));
			setAlertActions([{ label: 'OK', onPress: () => setAlertVisible(false) }]);
			setAlertVisible(true);
		}
	};
			...
			>
				...
				<View style={styles.loadingContainer}>
					<Text style={[styles.loadingText, { color: currentTheme.colors.text }]}>
						{t('notification.loading_settings')}
					</Text>
				</View>
			</SafeAreaView>
		);
						...
						<>
							...
							>
								...
								>
									<View style={styles.settingInfo}>
										...
										/>
										<Text
											style={[styles.settingText, { color: currentTheme.colors.text }]}
										>
											{t('notification.new_episodes')}
										</Text>
									</View>
									<Switch
								...
								<View style={styles.timingOptions}>
									{[1, 6, 12, 24].map((hours) => (
										...
										>
											...
											>
												{hours === 1
													? t('notification.hours_1')
													: `${hours} ${t('notification.hours_suffix')}`}
											</Text>
										</TouchableOpacity>
									))}
								</View>
							...
							>
								...
								>
									<View style={styles.statItem}>
										...
										/>
										<Text
											style={[styles.statLabel, { color: currentTheme.colors.textMuted }]}
										>
											{t('notification.stats_upcoming')}
										</Text>
									...
									<View style={styles.statItem}>
										...
										<Text
											style={[styles.statLabel, { color: currentTheme.colors.textMuted }]}
										>
											{t('notification.stats_this_week')}
										</Text>
										<Text style={[styles.statValue, { color: currentTheme.colors.text }]}>
								...
								>
									...
									>
										{isSyncing
											? t('notification.syncing')
											: t('notification.sync_button')}
									</Text>
								</TouchableOpacity>
							...
							>
								<Text
									style={[styles.sectionTitle, { color: currentTheme.colors.text }]}
								>
									{t('notification.section_advanced')}
								</Text>
								...
								>
									...
									/>
									<Text
										style={[styles.resetButtonText, { color: currentTheme.colors.error }]}
									>
										{t('notification.reset_button')}
									</Text>
								</TouchableOpacity>
