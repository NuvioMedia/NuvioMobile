...
import {
	...
	AboutFooter,
} from './settings/AboutSettingsScreen';
...
const Sidebar: React.FC<SidebarProps> = ({
	...
	return (
		...
		>
			...
			>
				<Text
					style={[styles.sidebarTitle, { color: currentTheme.colors.highEmphasis }]}
				>
					Settings
				</Text>
			</View>
...
const SettingsScreen: React.FC = () => {
	...
	const [developerModeEnabled, setDeveloperModeEnabled] =
	...
	const visibleCategories = SETTINGS_CATEGORIES.filter((category) => {
		if (settingsConfig?.categories?.[category.id]?.visible === false)
			return false;
		if (category.id === 'developer' && !__DEV__ && !developerModeEnabled)
			return false;
		...
		switch (categoryId) {
							...
							<SettingItem
								...
								customIcon={
									<FastImage
										...
										style={[
											styles.syncLogoIcon,
											isTablet ? styles.syncLogoIconTablet : null,
										]}
							...
							<SettingItem
								...
								description={
									isSimklAuthenticated
										? t('settings.items.simkl_connected')
										: t('settings.items.simkl_desc')
								}
					...
					<>
						<SettingsCard title="GENERAL" isTablet={isTablet}>
							<SettingItem
								...
								description={t(
									`settings.${LOCALES.find((l) => l.code === i18n.language)?.key}`,
								)}
								icon="globe"
								renderControl={() => <ChevronRight />}
				...
				return __DEV__ || developerModeEnabled ? (
					<SettingsCard title={t('settings.sections.testing')} isTablet={isTablet}>
						...
						<SettingItem
							...
							icon="trash-2"
							onPress={() => {
								openAlert(t('settings.clear_data'), t('settings.clear_data_desc'), [
									{ label: 'Cancel', onPress: () => {} },
									{
										label: 'Clear',
										onPress: async () => {
			...
			case 'cache':
				return mdblistKeySet ? (
					<SettingsCard
						title={t('settings.sections.cache_management')}
						isTablet={isTablet}
					>
						<SettingItem
			...
			case 'backup':
				return (
					<SettingsCard
						title={t('settings.backup_restore').toUpperCase()}
						isTablet={isTablet}
					>
						<SettingItem
			...
			case 'updates':
				return (
					<SettingsCard
						title={t('settings.updates').toUpperCase()}
						isTablet={isTablet}
					>
						<SettingItem
		...
		return (
			...
			>
				...
				>
					...
					>
						<Text
							style={[styles.bottomSheetTitle, { color: currentTheme.colors.white }]}
						>
		...
		>
			...
			<View style={{ flex: 1 }}>
				<View style={styles.contentContainer}>
					...
					>
						{/* Account */}
						{settingsConfig?.categories?.['account']?.visible !== false &&
							(showTraktItem || showSimklItem || showCloudSyncItem) && (
						...
						{/* Downloads Counter */}
						{settingsConfig?.items?.['downloads_counter']?.visible !== false &&
							displayDownloads !== null && (
								<View style={styles.downloadsContainer}>
									<Text
										style={[
											styles.downloadsNumber,
											{ color: currentTheme.colors.primary },
										]}
									>
