...
const DeveloperSettingsScreen: React.FC = () => {
	...
	const handleResetOnboarding = async () => {
		try {
			...
			openAlert(
				t('common.success'),
				t('settings.developer_settings.onboarding_reset_success'),
			);
		} catch (error) {
			openAlert(
				t('common.error'),
				t('settings.developer_settings.onboarding_reset_error'),
			);
		}
	};
	...
	const handleResetCampaigns = async () => {
		...
		openAlert(
			t('common.success'),
			t('settings.developer_settings.campaign_reset_success'),
		);
	};

	const handleClearAllData = () => {
		openAlert(
			t('settings.developer_settings.clear_all_data'),
			t('settings.developer_settings.clear_all_data_desc'),
			[
				{ label: t('common.cancel'), onPress: () => {} },
				{
					...
					onPress: async () => {
						try {
							...
							openAlert(
								t('common.success'),
								t('settings.developer_settings.clear_success'),
							);
						} catch (error) {
							openAlert(
								t('common.success'),
								t('settings.developer_settings.clear_error'),
							);
						}
					},
				},
		...
		>
			...
			>
				<SettingsCard title={t('settings.sections.testing')}>
					<SettingItem
						title={t('settings.developer_settings.plugin_tester_title')}
						description={t('settings.developer_settings.plugin_tester_desc')}
						icon="terminal"
