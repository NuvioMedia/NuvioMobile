...
export const PrivacySettingsContent: React.FC<PrivacySettingsContentProps> = ({
	...
	const showAlert = useCallback(
		(
			...
			const alertActions = (actions || [{ label: 'OK', onPress: () => {} }]).map(
				(action) => ({
					text: action.label,
	...
	const handlePiiToggle = async (enabled: boolean) => {
		if (enabled) {
			showAlert(t('privacy.enable_pii_title'), t('privacy.enable_pii_message'), [
				{ label: t('common.cancel', 'Cancel'), onPress: () => {} },
				{
					label: t('common.enable'),
					onPress: async () => {
	...
	const handleDisableAll = () => {
		showAlert(t('privacy.disable_all_title'), t('privacy.disable_all_message'), [
			{ label: t('common.cancel', 'Cancel'), onPress: () => {} },
			{
	...
	const handleResetToRecommended = async () => {
		...
		showAlert(t('privacy.reset_title'), t('privacy.reset_message'));
	};
	...
	return (
		<>
			...
			<SettingsCard
				title={t('privacy.section_error_reporting')}
				isTablet={isTablet}
			>
				<SettingItem
			...
			<SettingsCard title={t('privacy.section_learn_more')} isTablet={isTablet}>
				<SettingItem
					...
					icon="file-text"
					onPress={() =>
						Linking.openURL('https://tapframe.github.io/NuvioMobile/#privacy-policy')
					}
					renderControl={() => <ChevronRight />}
					isTablet={isTablet}
				/>
			...
			>
				<Text
					style={[styles.summaryTitle, { color: currentTheme.colors.highEmphasis }]}
				>
				...
				<View style={styles.summaryRow}>
					<View
						style={[
							styles.statusDot,
							{ backgroundColor: settings.analyticsEnabled ? '#4CAF50' : '#9E9E9E' },
						]}
					/>
					<Text
						style={[
				...
				<View style={styles.summaryRow}>
					<View
						style={[
							styles.statusDot,
							{
								backgroundColor: settings.errorReportingEnabled ? '#4CAF50' : '#9E9E9E',
							},
						]}
					/>
					<Text
				...
				<View style={styles.summaryRow}>
					<View
						style={[
							styles.statusDot,
							{
								backgroundColor: settings.sessionReplayEnabled ? '#FF9800' : '#9E9E9E',
							},
						]}
					/>
					<Text
					...
					>
						{t('privacy.summary_replay')}:{' '}
						{settings.sessionReplayEnabled
							? t('common.on', 'On')
							: t('common.off', 'Off')}
					</Text>
				</View>
				<View style={styles.summaryRow}>
					<View
						style={[
							styles.statusDot,
							{ backgroundColor: settings.piiEnabled ? '#FF9800' : '#9E9E9E' },
						]}
					/>
					<Text
						style={[
					...
					>
						{t('privacy.summary_pii')}:{' '}
						{settings.piiEnabled ? t('common.on', 'On') : t('common.off', 'Off')}
					</Text>
				</View>
				<Text
					style={[styles.restartNote, { color: currentTheme.colors.mediumEmphasis }]}
				>
