...
export const AboutSettingsContent: React.FC<AboutSettingsContentProps> = ({
	...
	const [internalDisplayDownloads, setInternalDisplayDownloads] = useState<
		number | null
	>(null);
	...
	const verifyDeveloperCode = async () => {
		...
		if (codeInput === expectedCode) {
			try {
				...
				showAlert(
					t('settings.developer_mode.enabled_title'),
					t('settings.developer_mode.enabled_message'),
				);
			} catch (error) {
			...
			showAlert(
				t('settings.developer_mode.invalid_code_title'),
				t('settings.developer_mode.invalid_code_message'),
			);
		}
		setCodeInput('');
	};
	...
	const handleDisableDeveloperMode = () => {
		showAlert(
			t('settings.developer_mode.disable_title'),
			t('settings.developer_mode.disable_message'),
			[
				{
				...
				},
				{
					...
					onPress: async () => {
						try {
							...
							showAlert(
								t('settings.developer_mode.disabled_title'),
								t('settings.developer_mode.disabled_message'),
							);
						} catch (error) {
							console.error('Failed to save developer mode state:', error);
						}
		...
		<>
			<SettingsCard title={t('settings.sections.information')} isTablet={isTablet}>
				...
				<SettingItem
					...
					onPress={() =>
						Linking.openURL(
							'https://tapframe.github.io/NuvioStreaming/#privacy-policy',
						)
					}
					renderControl={() => <ChevronRight />}
					isTablet={isTablet}
				...
				{developerModeEnabled && (
					<SettingItem
						title={t('settings.developer_mode.title')}
						description={t('settings.developer_mode.enabled_desc')}
						icon="code"
				...
				>
					...
					<View style={modalStyles.container}>
						<Text style={modalStyles.title}>
							{t('settings.developer_mode.enter_code_title')}
						</Text>
						<Text style={modalStyles.message}>
							{t('settings.developer_mode.enter_code_message')}
						</Text>
						<TextInput
