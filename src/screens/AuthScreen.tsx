...
const AuthScreen: React.FC = () => {
	...
	useEffect(() => {
		...
		Animated.sequence([
			Animated.parallel([
				...
				Animated.timing(ctaTextTranslateY, {
					toValue: -4,
					duration: 120,
					useNativeDriver: true,
				}),
	...
	const canSubmit =
		isEmailValid &&
		isPasswordValid &&
		(mode === 'signin' || (isConfirmValid && passwordsMatch));
	...
	const handleSubmit = async () => {
		...
		if (!isEmailValid) {
			...
			setError(msg);
			showError(
				t('auth_screen.invalid_email_title'),
				t('auth_screen.invalid_email_desc'),
			);
		...
		if (!isPasswordValid) {
			...
			setError(msg);
			showError(
				t('auth_screen.password_short_title'),
				t('auth_screen.password_short_desc'),
			);
			Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(
				() => {},
			);
		...
		if (mode === 'signup' && !passwordsMatch) {
			...
			setError(msg);
			showError(t('password_not_match_title'), t('password_not_match_desc'));
			Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(
				() => {},
			);
			return;
		...
		if (err) {
			if (
				mode === 'signup' &&
				err.startsWith(EMAIL_CONFIRMATION_REQUIRED_PREFIX)
			) {
				setError(null);
				setMode('signin');
				setPassword('');
			...
			setError(cleanError);
			showError(t('auth_screen.authentication_failed'), cleanError);
			Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error).catch(
				() => {},
			);
		} else {
			const msg =
				mode === 'signin' ? 'Logged in successfully' : 'Sign up successful';
	...
	return (
		<View style={{ flex: 1 }}>
			<LinearGradient colors={AUTH_BG_GRADIENT} style={StyleSheet.absoluteFill} />
				...
				>
					...
					>
						...
						>
							...
							>
								{mode === 'signin'
									? t('auth_screen.welcome_back')
									: t('auth_screen.create_account')}
							</Animated.Text>
							{keyboardHeight === 0 && (
								<Text
									style={[styles.subheading, { color: currentTheme.colors.textMuted }]}
								>
									{t('auth_screen.sync_desc')}
								</Text>
							)}
						</Animated.View>
						...
						<Animated.View
							style={[
								...
								keyboardHeight > 0 ? styles.cardCompact : null,
								{
									backgroundColor:
										Platform.OS === 'android' ? '#121212' : 'rgba(255,255,255,0.02)',
									borderColor:
										Platform.OS === 'android' ? '#1f1f1f' : 'rgba(255,255,255,0.06)',
									...(Platform.OS !== 'android'
										? {
						...
						>
							...
							<View style={styles.inputContainer}>
								...
								>
									...
									<TextInput
										placeholder={t('auth_screen.password_min_placeholder')}
										placeholderTextColor="rgba(255,255,255,0.4)"
										style={[styles.input, { color: currentTheme.colors.white }]}
										autoCapitalize="none"
							...
							{mode === 'signup' && (
								<View style={styles.inputContainer}>
									...
									>
										...
										<TextInput
											placeholder={t('auth_screen.password_confirm_placeholder')}
											placeholderTextColor="rgba(255,255,255,0.4)"
											style={[styles.input, { color: currentTheme.colors.white }]}
											autoCapitalize="none"
										...
										<Animated.Text
											style={[
												styles.ctaText,
												{
													...
													transform: [{ translateY: ctaTextTranslateY }],
												},
											]}
										>
											{mode === 'signin'
							...
							>
								...
								>
									{mode === 'signin'
										? t('auth_screen.dont_have_account')
